...
const transConfig = (config: any[], type: FieldType[]) => {
  return config.map((item) => {
    if (item.children) {
      item.children = transConfig(item.children, type);
    }
    return {
      ...item,
      dependencies: [...(item.dependencies || []), "schema"],
      hidden: (...args: HiddenFnParams) => {
        ...
        console.log(
          ...
          args[1],
          originHiddenRes,
          getSchemaItem(...args).fieldTypeNotIncludes(type),
          type,
        );

        return originHiddenRes;
        return (
          originHiddenRes && getSchemaItem(...args).fieldTypeNotIncludes(type)
        );
      },
    };
  });
...
function generatePanelPropertyConfig(
  ...
  return {
    ...
    contentChildren: mergeWidgetConfig(
      [
        {
          sectionName: "数据",
          children: [
    ...
    styleChildren: mergeWidgetConfig(
      ...
      transConfig(AntdInputWidget.getPropertyPaneStyleConfig(), INPUT_TYPES),
    ),
  } as PanelConfig;
}
