...
import {
  ...
  CUSTOM_ROLE_TEXT,
  BUSINESS_PRICE_URL,
  CREATE_USERS_SUBMIT_SUCCESS,
  CREATE_USER_SUBMIT_SUCCESS,
    ...
    if (submitSucceeded) {
      toast.show(
        numberOfUsersInvited > 1
          ? createMessage(CREATE_USERS_SUBMIT_SUCCESS)
          : createMessage(CREATE_USER_SUBMIT_SUCCESS),
        { kind: "success" },
      );
    }
  }, [submitSucceeded]);
    ...
    <WorkspaceInviteWrapper>
        ...
        onSubmit={handleSubmit((values: any, dispatch: any) => {
          ...
          Promise.all(
            users.split(",").map(async (email) => {
              ...
              return await UserApi.syncSaveUserForPhal({
                userName: email,
                userPassword: password,
                userId: (data as any).id,
                groupId: props.workspaceSlug,
      ...
      >
        ...
        <StyledInviteFieldGroup>
          <div style={{ width: "100%" }}>
            <Input.Password
              data-testid="t--invite-password-input"
              name="password"
              onChange={(e) => {
        ...
        <div>
          <Button
            ...
            isDisabled={!valid || selectedOption.length === 0 || !password}
