...
import { ConfigProvider, Divider, Dropdown, theme } from "antd";
...
import type { MyRoute } from "./components/navList";
import PageNavList from "./components/navList";
...
export default (props: SidebarProps) => {
  ...
  const [currentRoute, setCurrentRoute] = useState<MyRoute>();
        ...
        menudata = current?.treeData.map((itdata: any, itIdx: number) => {
          return mapClearTree(
            ...
            (item: any) => {
              ...
              if (path.includes(pathname) && !currentRoute) {
                res.isInitRoute = true;
                setCurrentRoute(res);
              }
              delete res.children;
              return res;
      ...
      <ProConfigProvider hashed={false}>
        ...
        >
          <ProLayout
            ...
            route={{ routes: initState.menudata }}
            // bgLayoutImgList={[
            //   {
            ...
            menuItemRender={(item, dom) => (
              <div
                onClick={() => {
                  ...
                  if (item.isPage) {
                    setCurrentRoute(item as any);
                    goToPath(item.path || "/welcome");
                  }
                }}
              >
                {dom}
          ...
          >
            <PageContainer
              ...
              token={{
                paddingBlockPageContainerContent: 4,
